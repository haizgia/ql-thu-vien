...
<div class="container m-5">
    <div class="row">
        @if (!isset($data))
            @foreach ($dm as $d)
                <div class="col-lg-3">
                    <a href="{{route('search.danhmuc', ['id' => $d->maloai])}}">
        ...
        @else
            <div class="col-lg-3">
                @foreach ($dm as $d)
                    <a class="btn btn-link d-block w-100 @if ($d->maloai == $getCategory['maloai']) btn-primary @endif"
                        href="{{route('search.danhmuc', ['id' => $d->maloai])}}" style="text-align: left">
                        <i class="fa fa-arrow-right text-primary me-2"></i>
                        {{$d->tenloai}}
                    </a>
                @endforeach
